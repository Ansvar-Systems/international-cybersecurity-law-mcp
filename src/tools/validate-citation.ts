...
export function validateCitation(db: Database.Database, args: Args) {
  ...
  `).get(`%${sourceRef}%`, `%${sourceRef}%`) as any;
  ...
  `).get(source.id, articleNum) as any;
  ...
  if (!article) {
    ...
    for (const prefix of prefixes) {
      article = db.prepare(`
        SELECT * FROM articles WHERE source_id = ? AND article_number = ?
      `).get(source.id, `${prefix}${articleNum}`) as any;
      if (article) break;
    }
  }
