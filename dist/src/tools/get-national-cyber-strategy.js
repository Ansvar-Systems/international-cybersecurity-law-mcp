export function getNationalCyberStrategy(db, args) {
    let strategy;
    ...
    if (!strategy) {
        ...
        throw new Error(`Strategy not found. Available countries: ${available.map((a) => `${a.country_name} (${a.country_code})`).join(', ')}`);
    }
    ...
    return {
        strategy,
        _citation: {
            canonical_ref: `National Cybersecurity Strategy — ${strategy.country_name} (${strategy.country_code})`,
            lookup: { tool: 'get_national_cyber_strategy', args: { country_code: strategy.country_code } },
        },
        _meta: {
